...
HABIT-DATA contains the results of `org-habit-parse-todo`."
  (with-current-buffer
    ...
    (let* ((date (calendar-current-date))
           ;; Получаем текущий месяц (например, 2 для февраля)
...
(defvar org-habit-stats-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "C-v")   'org-habit-stats-calendar-scroll-left-three-months)
    (define-key map (kbd "M-v")   'org-habit-stats-calendar-scroll-right-three-months)
    (define-key map (kbd "C-x ]")   'org-habit-stats-calendar-forward-year)
    (define-key map (kbd "C-x [")   'org-habit-stats-calendar-backward-year)
